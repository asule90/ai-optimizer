...
| **QMD** *or* **Graphify** | Project context (pick one) | **QMD**: semantic search over `docs/**/*.md`. **Graphify**: knowledge graph for larger codebases/monorepos |
...
- **Graphify** — best for larger codebases or monorepos where you need relationship queries across code (`graphify query`, `graphify path`). Build once with `graphify .`.
...
**Graphify** (codebase relationships)

When `DOCS_TOOL=graphify`, the script installs Graphify and writes Cursor rules. Run `graphify .` once, then use `graphify query` / `graphify path` for structural questions in large repos.
